...
import { updateVenueDetails } from '../../actions/adminActions';
import {
    X,
    Save,
    Loader2,
...
// Helper utility: Resizes and compresses local uploads to prevent payload bloat
...
export const EditVenueModal: React.FC<EditVenueModalProps> = ({ venue, onClose, onRefresh }) => {
    // Declare hooks unconditionally at top level
    ...
    // Track previous venue ID for render-phase state synchronization
    const [prevVenueId, setPrevVenueId] = useState<string | null>(null);

    // Render-phase state adjustment when venue prop changes (prevents react-hooks/set-state-in-effect)
    if (venue && venue.id !== prevVenueId) {
        setPrevVenueId(venue.id);
        setName(venue.name || '');
        setBasePrice(venue.base_rental_rate_php || 0);
        setSlotDurationText(venue.slot_duration_text || '/ 5-hr slot block');
        setCapacity(venue.max_guest_capacity || 0);
        setDescription(venue.description || '');
        setImageUrls(venue.image_urls && venue.image_urls.length > 0 ? venue.image_urls : []);
        setIsActive(!venue.is_under_maintenance);
    }

    if (!venue) return null;

    // Handles local file upload with canvas downscaling
    ...
    const handleSubmit = (e: React.FormEvent<HTMLFormElement>) => {
        ...
        if (imageUrls.length === 0) {
            setFeedback({ type: 'error', text: 'Please add at least one cover photo for the venue.' });
            return;
        }

        startTransition(async () => {
            const res = await updateVenueDetails(venue.id, {
                name,
            ...
            if (res.success) {
                setFeedback({ type: 'success', text: res.message || 'Venue details updated successfully!' });
                setTimeout(() => {
                    onRefresh();
                    onClose();
                }, 1000);
            } else {
                setFeedback({ type: 'error', text: res.message || 'Failed to update venue.' });
            }
        });
    };
            ...
            <div className="bg-white rounded-2xl sm:rounded-3xl border border-stone-200 shadow-2xl max-w-lg w-full max-h-[92vh] overflow-y-auto">
                ...
                <div className="bg-emerald-950 px-5 py-4 text-white flex justify-between items-center border-b border-emerald-900 sticky top-0 z-10">
                    <div>
                        ...
                        <h3 className="font-serif text-lg sm:text-xl font-bold">Edit {venue.name}</h3>
                    </div>
                    <button
                        onClick={onClose}
                        type="button"
                ...
                <form onSubmit={handleSubmit} className="p-5 space-y-4 text-xs">
                    ...
                    <div className="bg-stone-50 p-3.5 rounded-xl border border-stone-200 flex items-center justify-between">
                        <div>
                            <span className="font-bold text-stone-900 block text-xs">Venue Availability Status</span>
                            <span className="text-[11px] text-stone-500">
                                {isActive ? 'Publicly active for bookings' : 'Hidden / Under Maintenance'}
                            </span>
                        </div>
                    ...
                    <div className="space-y-2.5 pt-1 border-t border-stone-100">
                        <div className="flex items-center justify-between">
                            <label className="font-bold text-stone-900 text-xs flex items-center gap-1.5">
                                ...
                                <span>Venue Showcase Photos ({imageUrls.length})</span>
                            </label>
                            {isCompressing && (
                        ...
                        {imageUrls.length > 0 ? (
                            ...
                            <p className="text-[11px] text-stone-400 italic bg-stone-50 p-3 rounded-xl border border-stone-200 text-center">
                                No photos added yet. Upload local images or add web links below.
                            </p>
                        )}
                    ...
                    <div className="pt-3 grid grid-cols-2 gap-2.5 border-t border-stone-100">
                        ...
                        >
                            {isPending ? <Loader2 className="w-3.5 h-3.5 animate-spin" /> : <Save className="w-3.5 h-3.5 text-amber-300" />}
                            Save Venue
                        </button>
                    </div>
                </form>
