...
import { CheckCircle2, Clock, XCircle, ShieldCheck, Filter, ArrowUpDown } from 'lucide-react';
...
export const BookingsTab: React.FC<BookingsTabProps> = ({ bookings, onRefresh }) => {
    ...
    const getStatusBadge = (status: EventBookingStatus) => {
        switch (status) {
            case 'confirmed':
                return <span className="bg-emerald-100 text-emerald-900 border border-emerald-300 text-[10px] font-bold px-2.5 py-1 rounded-full uppercase">Confirmed</span>;
            case 'pending_deposit':
                return <span className="bg-amber-100 text-amber-900 border border-amber-300 text-[10px] font-bold px-2.5 py-1 rounded-full uppercase">Pending Deposit</span>;
            case 'completed':
                return <span className="bg-blue-100 text-blue-900 border border-blue-300 text-[10px] font-bold px-2.5 py-1 rounded-full uppercase">Completed</span>;
            case 'cancelled':
                return <span className="bg-red-100 text-red-900 border border-red-300 text-[10px] font-bold px-2.5 py-1 rounded-full uppercase">Cancelled</span>;
        }
    };
            ...
            <div className="flex flex-wrap items-center justify-between gap-4 bg-white p-4 rounded-2xl border border-stone-200">
                ...
                <div className="flex flex-wrap gap-2 text-xs font-semibold">
                    {(['all', 'pending_deposit', 'confirmed', 'completed', 'cancelled'] as const).map((st) => (
                        ...
                        >
                            {st === 'all' ? 'All Bookings' : st.replace('_', ' ').toUpperCase()}
                        </button>
                    ))}
                </div>
            </div>
                    ...
                    <div className="overflow-x-auto">
                        <table className="w-full text-left text-xs border-collapse">
                            ...
                            <tr className="bg-stone-50 border-b border-stone-200 text-stone-500 uppercase tracking-wider font-bold">
                                ...
                                <th className="py-3.5 px-4">Deposit (30%)</th>
                            ...
                            {filteredBookings.map((b) => (
                                <tr key={b.id} className="hover:bg-stone-50/80 transition-colors">
                                    <td className="py-4 px-4 font-bold text-stone-900 font-mono">{b.id}</td>
                                    <td className="py-4 px-4">
                                        <p className="font-bold text-stone-900">{b.organizer_name}</p>
                                        <p className="text-[11px] text-stone-500">{b.organizer_phone}</p>
                                    </td>
                                    <td className="py-4 px-4">
                                        <p className="font-semibold text-stone-900">{b.event_date}</p>
                                        <p className="text-[11px] text-emerald-800 capitalize">{b.slot_block.replace('_', ' ')}</p>
                                    </td>
                                    ...
                                    <td className="py-4 px-4 text-right">
                                        <button
                                            ...
                                            className="bg-stone-100 hover:bg-emerald-900 hover:text-white text-stone-800 border border-stone-200 px-3 py-1.5 rounded-xl transition-all font-semibold text-[11px]"
                                        >
                                            Verify / Review
                                        </button>
                                    </td>
